...
assert_contains "$SKILL" "no issue-tracker context" "description excludes tracker-free bug/feature/support asks"
assert_not_contains "$SKILL" "issue number, bug report, feature request, support request" "description does not claim bare bug/feature/support triggers"
...
assert_contains "$SKILL" "Use two-step approval" "skill requires two-step mutation approval"
...
assert_contains "$SCENARIOS" "blanket approval" "mutation scenario pressures blanket approval"
assert_contains "$SCENARIOS" "exact draft" "mutation scenario expects exact draft approval"
assert_contains "$SCENARIOS" "Local bug with no tracker context" "scenarios include tracker-free bug negative case"
assert_contains "$SCENARIOS" "Feature idea with no tracker context" "scenarios include tracker-free feature negative case"
assert_contains "$SCENARIOS" "must not trigger" "scenarios mark negative trigger cases"
