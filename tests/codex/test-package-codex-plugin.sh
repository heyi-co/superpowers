...
tar_mtimes="$(python3 - "$tar_archive" <<'PY'
import sys
import tarfile
...
assert_equals "$tar_mtimes" "0" "tar.gz archive normalizes entry timestamps"
...
worktree_archive="$TEST_ROOT/worktree.zip"
git clone -q --no-local "$REPO_ROOT" "$worktree_repo"
git -C "$worktree_repo" worktree add -q "$linked_worktree" HEAD
if output="$("$linked_worktree/scripts/package-codex-plugin.sh" --allow-dirty --metadata-source "$metadata_source" --output "$worktree_archive" 2>&1)"; then
  pass "package script accepts linked git worktree checkout"
else
  fail "package script accepts linked git worktree checkout"
  printf '%s\n' "$output" | sed 's/^/      /'
fi
