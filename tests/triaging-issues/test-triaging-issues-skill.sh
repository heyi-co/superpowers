...
assert_contains "$SKILL" "## Complexity" "skill defines complexity routing hint"
assert_contains "$SKILL" "Complexity:" "triage result template includes complexity field"
assert_contains "$SKILL" "advisory only" "complexity is advisory and dispatcher may override"
assert_contains "$SCENARIOS" "Complexity is a routing hint, not permission" "scenarios cover complexity semantics"
